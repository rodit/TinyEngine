    ...
    public class MapConverter
    {
        ...
        public static bool ConvertMap(string input, string output)
        {
            ...
            foreach (MapLayer layer in map.Layers)
            {
                if (layer is ObjectLayer && layer.Name == "collisions")
                {
                    ...
                    foreach (MapObject obj in ol.Objects)
                    {
                        ...
                        rotg.DrawImage(img, new RectangleF(obj.X, obj.Y, obj.Width, obj.Height), new RectangleF(obj.X, obj.Y, obj.Width, obj.Height), GraphicsUnit.Pixel);
                    }
                }
            }
            g.Dispose();
            ...
            if (rot != null)
            {
                ...
                File.WriteAllBytes("cache/rot.png", rotMem.ToArray());
                rotMem.Dispose();
            }
